...
HeadingPlanner::HeadingPlanner(rclcpp::Node::SharedPtr nh) {
  node_ = nh;
  ...
  node_->declare_parameter("heading_planner.yaw_diff", 0.1);
  node_->declare_parameter("heading_planner.lambda1", 1.0);
  node_->declare_parameter("heading_planner.lambda2", 1.0);
  node_->declare_parameter("heading_planner.half_vert_num", 5);
  node_->declare_parameter("heading_planner.max_yaw_rate", 1.0);
  node_->declare_parameter("heading_planner.w", 10.0);
  node_->declare_parameter("heading_planner.weight_type", 1);

  node_->get_parameter("heading_planner.yaw_diff", yaw_diff_);
  node_->get_parameter("heading_planner.lambda1", lambda1_);
  node_->get_parameter("heading_planner.lambda2", lambda2_);
  node_->get_parameter("heading_planner.half_vert_num", half_vert_num_);
  node_->get_parameter("heading_planner.max_yaw_rate", max_yaw_rate_);
  node_->get_parameter("heading_planner.w", w_);
  node_->get_parameter("heading_planner.weight_type", weight_type_);
...
void HeadingPlanner::setMap(const GridMap::Ptr& map) { grid_map_ = map; }
  ...
  for (int x = lbi[0]; x <= ubi[0]; ++x)
    for (int y = lbi[1]; y <= ubi[1]; ++y)
      for (int z = lbi[2]; z <= ubi[2]; ++z) {
        ...
        pt_idx << x, y, z;
        if (!grid_map_->isInMap(pt_idx) || !grid_map_->isUnknown(pt_idx)) continue;
        
        grid_map_->indexToPos(pt_idx, check_pt);
        if (!insideFoV(check_pt, pt, normals)) continue;

        char flag = cast_flags_.getFlag(pt_idx);
        if (flag == 1) {  
          if (weight_type_ == UNIFORM) gain += 1;
          ...
          char result = 1;
          casters_[task_id]->setInput(pt, check_pt);
          while (casters_[task_id]->step(ray_pt)) {
            Eigen::Vector3i ray_id;
            grid_map_->posToIndex(ray_pt, ray_id);
            if (grid_map_->getInflateOccupancy(ray_id) == 1) {
              result = 2;
              break;
            }
          }
...
void HeadingPlanner::initCastFlag(const Eigen::Vector3d& pos) {
  ...
  Eigen::Vector3i lbi, ubi;
  grid_map_->posToIndex(pos - vec, lbi);
  grid_map_->posToIndex(pos + vec, ubi);
...
void HeadingPlanner::calcFovAABB(const Eigen::Matrix3d& R_wc, const Eigen::Vector3d& t_wc,
  ...
  grid_map_->posToIndex(lbd, lb);
  grid_map_->posToIndex(ubd, ub);
}
...
bool HeadingPlanner::insideFoV(const Eigen::Vector3d& pw, const Eigen::Vector3d& pc,
  ...
  if (dir.norm() > far_) return false;
  for (auto n : normals) {
    if (dir.dot(n) < 0.1) return false;
  }
  return true;
}
...
void HeadingPlanner::calcVisibFrontier(const Eigen::Vector3d& pt, const double& yaw,
  ...
  for (auto& idx : point_idx) {
    ...
    if (insideFoV(pw)) visib_idx[idx] = 1;
  }
}
...
bool HeadingPlanner::insideFoV(const Eigen::Vector4d& pw) {
  Eigen::Vector4d pc;
  if (pc(2) < near_ || pc(2) > far_) return false;
  double txz = pc(0) / pc(2);
  if (txz > tanxz_ || txz < -tanxz_) return false;
  double tyz = pc(1) / pc(2);
  if (tyz > tanyz_ || tyz < -tanyz_) return false;
  return true;
}
