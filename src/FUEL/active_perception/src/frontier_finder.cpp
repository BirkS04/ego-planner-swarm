    ...
    for (int y = min_id(1); y <= max_id(1); ++y) {
      for (int z = min_id(2); z <= max_id(2); ++z) {
        ...
        if (knownfree(cur)) {
          ...
          if (frontier_flag_[toadr(cur)] == 0 && isNeighborUnknown(cur)) {
            count_is_frontier++;
            expandFrontier(cur);
            count_expanded++;
          }
...
void FrontierFinder::expandFrontier(const Eigen::Vector3i& first) {
  ...
  if (expanded.size() > 0) {
    if ((int)expanded.size() > cluster_min_) {
      ...
      // WICHTIGER FIX: Flags zurücksetzen, sonst blockiert dieser "Müll" zukünftige Suchen!
...
bool FrontierFinder::isNearUnknown(const Eigen::Vector3d& pos) {
  ...
  for (int x = -vox_num; x <= vox_num; ++x) {
    for (int y = -vox_num; y <= vox_num; ++y) {
      for (int z = -1; z <= 1; ++z) {
        ...
        // Zähle Voxel, die außerhalb der Karte oder Unknown sind
  ...
  // ====================================================================
  // LIDAR-FIX: Toleranz für den "Schweizer Käse" Effekt.
  // Nur wenn mehr als 15% der Box Unknown sind, ist es WIRKLICH der Map-Rand.
  // Ein paar winzige Löcher im Freiraum ignorieren wir!
  // ====================================================================
